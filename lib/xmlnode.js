...
XmlNode.prototype.createSaxParser = function (options) {
  ...
  parser.onopencdata = function () {
    record.hasCDATA = true
  }

  parser.ontext = function(value) {
    if (record && record.hasCDATA === true) {
      return // ignore, avoiding overwrite (e.g. `<foo><![CDATA[def]]>\r\n</foo>`)
    }
    if (record) {
  ...
  parser.oncdata = function (value) {
    if (record) {
      if (record.CDATAStarted === true) { // Avoid chars before CDATA (like whitespaces) (e.g. `<foo>\r\n<![CDATA[def]]></foo>`)
        elem.concatText(record, value);
      } else {
        elem.addText(record, value);
        record.CDATAStarted = true
      }
    }
  };
